...
async def get_current_user(
    token: Annotated[str, Depends(oauth2_scheme)],  # note token str type, not Token type, OAuth2PasswordBearer returns only the raw access token string from auth.py, 
    db: Annotated[AsyncSession, Depends(get_db)],
):
